...
public:
	Camera(Vector3 position, Vector3 direction, Vector3 up,int imageWidth, int imageHeight, float fov)
		: m_position(position), m_direction(direction), m_up(up), imageWidth(imageWidth), imageHeight(imageHeight), fov(fov), samplesPerPixel(1024)	
		{
			// 计算摄像机画布
			double theta = fov; // 视角角度(水平)
			double halfTheta = theta / 2.0; // 视角角度的一半
			halfTheta = halfTheta * BickConstants::PI / 180.0; // 转换为弧度
...
private:
	...
	int samplesPerPixel = 1024; // 采样数 (现在通过配置系统控制)
